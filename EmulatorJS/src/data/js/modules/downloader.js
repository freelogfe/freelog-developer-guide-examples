...
export class Downloader {
    ...
    downloadGameCore() {
        ...
        if (!this.emulator.config.threads && this.emulator.coreManager.requiresThreads(this.emulator.coreManager.getCore())) {
        ...
        if (!this.emulator.supportsWebgl2 && this.emulator.coreManager.requiresWebGL2(this.emulator.coreManager.getCore())) {
            ...
            this.emulator.checkCompression(new Uint8Array(data), this.emulator.localization("Decompress Game Core")).then((data) => {
                ...
                for (let k in data) {
                    if (k.endsWith(".wasm")) {
                        ...
                        this.emulator.coreManager.checkCoreCompatibility(JSON.parse(new TextDecoder().decode(data[k])));
                    } else if (k === "core.json") {
                ...
                this.emulator.coreManager.initGameCore(js, wasm, thread);
            });
        }
        let version = this.emulator.ejs_version.endsWith("-beta") ? "nightly" : this.emulator.ejs_version;
        const report = "cores/reports/" + this.emulator.coreManager.getCore() + ".json";
        this.downloadFile(`https://cdn.emulatorjs.org/${version}/data/${report}`, null, true, { responseType: "text", method: "GET" }).then(async rep => {
            ...
            if (this.emulator.coreManager.requiresWebGL2(this.emulator.coreManager.getCore())) {
            ...
            let filename = this.emulator.coreManager.getCore() + (threads ? "-thread" : "") + legacy + "-wasm.data";
            if (!this.emulator.debug) {
