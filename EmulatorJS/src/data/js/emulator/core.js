...
export function getCores(emulator) {
    let rv = {
        "atari5200": ["a5200"],
        "vb": ["beetle_vb"],
...
export function getCore(emulator, generic) {
    ...
    if (cores[gen] && cores[gen].includes(emulator.preGetSetting("retroarch_core"))) {
        return emulator.preGetSetting("retroarch_core");
    }
    if (cores[core]) {
        return cores[core][0];
    }
