// DOM and event utilities for EmulatorJS

function createElement(emulator, type) {
    return document.createElement(type);
}

function addEventListener(emulator, element, listener, callback) {
...
function removeEventListener(emulator, data) {
    for (let i = 0; i < data.length; i++) {
        data[i].elem.removeEventListener(data[i].listener, data[i].cb);
    }
}

function versionAsInt(emulator, ver) {
        ...
        if (response.ok) {
            response.text().then(body => {
                ...
                if (versionAsInt(emulator, emulator.ejs_version) < versionAsInt(emulator, version.version)) {
                    console.log(`Using EmulatorJS version ${emulator.ejs_version} but the newest version is ${version.current_version}\nopen https://github.com/EmulatorJS/EmulatorJS to update`);
                }
            })
        }
    })
